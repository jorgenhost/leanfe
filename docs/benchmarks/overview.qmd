...
- **YOCO compression + sparse matrices** for low/medium-cardinality FEs (< 10K levels per FE, < 20K total)
- **FWL iterative demeaning** for high-cardinality FEs (> 10K levels per FE or > 20K total)

Both approaches produce **identical results** to traditional methods, just faster.

::: {.callout-tip}
## Comprehensive Benchmarks
For detailed benchmarks across **all 4 backend/language combinations** (Python/Polars, Python/DuckDB, R/Polars, R/DuckDB) and **all dataset sizes** (100K to 100M), see [Comprehensive Benchmarks](comprehensive.qmd).
:::
...
def run_benchmark(n_obs, include_pyfixest=True):
    ...
    results["strategy"] = result_polars.get("strategy", "compress" if result_polars.get("n_compressed") else "fwl")
...
print("=" * 105)
print(f"{'Obs':>12} {'leanfe-Polars':>14} {'leanfe-DuckDB':>14} {'pyfixest':>12} {'Speedup':>10} {'Strategy':>12}")
print("-" * 105)
...
for n in sizes:
    ...
    strategy = result.get('strategy', 'compress')
    print(f"{n:>12,} {result['py_polars_time']:>13.2f}s {result['py_duckdb_time']:>13.2f}s {pyfixest_str:>12} {speedup:>10} {strategy:>12}")
...
cat(strrep("=", 105), "\n")
cat(sprintf("%12s %14s %14s %12s %10s %12s\n", "Obs", "leanfe-Polars", "leanfe-DuckDB", "fixest", "Speedup", "Strategy"))
cat(strrep("-", 105), "\n")

for (n in sizes) {
  result <- run_r_benchmark(n)
  speedup <- result$fixest_time / result$r_polars_time
  strategy <- if (!is.null(result$compression) && result$compression < 1) "compress" else "fwl"
  cat(sprintf("%12s %13.2fs %13.2fs %11.2fs %9.1fx %12s\n", 
